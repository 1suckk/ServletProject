...
<div style="margin: 20px;">
	...
	<c:set var="num1" value="45.6789"></c:set>
	<c:set var="num2" value="27.8"></c:set>
	
	<h5>
		<!-- pattern="0.00" : 무조건 두 자리로 맞춤 -->
		숫자(소수점 이하 자릿수 2자리):
		<fmt:formatNumber value="${num1}" pattern="0.00"></fmt:formatNumber>,
		<fmt:formatNumber value="${num2}" pattern="0.00"></fmt:formatNumber>
	</h5>
	<h5>
		<!-- pattern="0.##" : 두자리로 맞추는데 값이 없을 경우 출력 안됨 -->
		숫자 (소수점 이하 자릿수 2자리):
		<fmt:formatNumber value="${num1}" pattern="0.##"></fmt:formatNumber>.
		<fmt:formatNumber value="${num2}" pattern="0.##"></fmt:formatNumber>
	</h5>
</div>	
</body>
</html>